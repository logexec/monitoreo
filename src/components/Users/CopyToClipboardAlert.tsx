...
const CopyToClipboardAlert = () => {
  ...
  return (
    <Alert className="py-4 px-2 rounded-md bg-sky-50 border border-sky-200">
      <div className="flex items-center space-x-2">
        <Info className="h-5 w-5 text-sky-600" />
        <AlertTitle className="text-sky-800">Recuerda</AlertTitle>
      </div>
      <AlertDescription className="mt-1.5 flex items-center justify-center space-x-2">
        <span className="text-sm text-gray-700">
          La contraseña por defecto es
        </span>
        <div className="relative border rounded border-sky-200 px-0.5 py-1 w-40">
          <input
            type="text"
            value={password}
            readOnly
            className="bg-transparent focus:outline-none cursor-pointer text-sm text-sky-600 pl-4"
          />
          <button
